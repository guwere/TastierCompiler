...
public class Scanner {
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 3:
				if (ch == 9 || ch >= ' ' && ch <= '!' || ch >= '#' && ch <= '_' || ch >= 'a' && ch <= '{' || ch >= '}' && ch <= '~' || ch == 163) {AddCh(); goto case 3;}
